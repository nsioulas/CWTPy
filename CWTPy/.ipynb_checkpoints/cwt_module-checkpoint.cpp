...
//
// This version uses FFTW_MEASURE for planning and allows the user to choose the
// scale distribution via a new parameter 'scale_type'. Supported presets include:
//   - "log": Exponentially spaced scales: [s0 * 2^(1/nv), s0 * 2^(2/nv), ...]
//   - "log-piecewise": Similar to "log" but with high scales downsampled to avoid redundancy.
//   - "linear": Linearly spaced scales between s0 and s1.
// (Other presets could be added as needed.)
// 
...
static std::vector<double> generate_scales(double s0, double s1, int nv, const std::string &scale_type) {
    if (scale_type == "log") {
        ...
        // First generate a full log scale.
        ...
        // Linearly spaced scales: choose number of scales as nv * number of octaves.
...
/*
   ...
   New parameter:
     scale_type: string specifying the distribution of scales.
       Supported options: "log", "log-piecewise", "linear".
...
py::tuple cwt_morlet_full(
    ...
    // Determine minimal and maximal scales (in seconds) from frequency bounds.
    double smin = freq_to_scale(max_freq, omega0);  // smallest scale corresponds to highest frequency
    double smax = freq_to_scale(min_freq, omega0);    // largest scale corresponds to lowest frequency
    if (smin >= smax)
        throw std::runtime_error("Scale range is invalid. Check freq bounds.");
    // Generate scales based on the user-specified scheme.
